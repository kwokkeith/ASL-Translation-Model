...
def load_model(weights_path, dataset_path, learning_rate=0.001):
    """Loads a trained model with given weights and dataset labels."""
    actions = sorted([
    ...
    optimizer = extract_optimizer_from_path(weights_path, learning_rate)
    model = build_model(num_classes=num_classes)
    model.compile(optimizer=optimizer, loss='sparse_categorical_crossentropy', metrics=['accuracy'])
    model.load_weights(weights_path)
    
    print("Model loaded successfully with weights from:", weights_path)
...
def main():
    ...
    model, actions = load_model(args.weights, args.dataset, args.rate)
