...
from tensorflow.keras.layers import LSTM, Dense, Input, \
    SpatialDropout1D, BatchNormalization, Dropout
...
def build_lstm_model(input_shape=(15, 1662), num_classes=5):
    """
    Builds and compiles an LSTM model for action recognition.
    ...
    model.add(LSTM(64, return_sequences=True, recurrent_dropout=0.25, dropout=0.3, activation='relu'))
    model.add(BatchNormalization())
    
    model.add(LSTM(128, return_sequences=True, recurrent_dropout=0.25, dropout=0.3, activation='relu'))
    model.add(BatchNormalization())
    
    model.add(LSTM(64, return_sequences=False, recurrent_dropout=0.25, dropout=0.3, activation='relu'))
    model.add(BatchNormalization())
...
def build_static_model(input_shape=(42,), num_classes=36):
    model = Sequential()
    model.add(Input(shape=input_shape))
    model.add(BatchNormalization())
    model.add(Dense(128, activation='mish', kernel_regularizer=l2(0.001)))
    model.add(Dropout(0.5))
    model.add(Dense(64, activation='mish', kernel_regularizer=l2(0.001)))
    model.add(Dropout(0.5))
    model.add(Dense(32, activation='mish', kernel_regularizer=l2(0.001)))
...
def extract_keypoints_static(results):
    """Extract Keypoints from mediapipe in form of np array"""
    # pose = np.array([[res.x, res.y, res.z, res.visibility]
    #                  for res in results.pose_landmarks.landmark]) \
    #     .flatten() if results.pose_landmarks else np.zeros(33*4)
