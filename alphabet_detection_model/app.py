...
datasetdir = base_dir + "/model/dataset/numbers"
...
def main():
    ...
    while True:
        ...
        if mode == 2:
            ...
            loading_img = cv.imread(base_dir + "/assets/1.jpg", cv.IMREAD_COLOR)
            ...
            for imgclass in os.listdir(datasetdir):
                ...
                print(f"Processing class '{imgclass}' with label {imglabel}...")
                        ...
                        for _ in [1, 2]:
                            ...
                            if results.multi_hand_landmarks is not None:
                                for hand_landmarks, handedness in zip(
                                    ...
                                    print("Logging imglabel: ", imglabel)
                            img = cv.flip(img, 0)
            ...
            if results.multi_hand_landmarks is not None:
                for hand_landmarks, handedness in zip(
                    ...
                    string_words(keypoint_classifier_labels[hand_sign_id])
...
def logging_csv(number, mode, landmark_list):
    ...
    if (mode == 1 or mode == 2) and (0 <= number <= 35):
        csv_path = base_dir + "/model/keypoint_classifier/keypoint.csv"
...
def draw_landmarks(image, landmark_point):
    ...
    # Key Points
    for landmark in enumerate(landmark_point):
...
# function to string words together. word is being printed in draw_info() function
def string_words(hand_sign_text):
    global word, consecutive_frames, prev_sign
    
    # Check if the detected letter is the same as the previous frame
    if hand_sign_text == prev_sign:
        consecutive_frames += 1
    else:
        consecutive_frames = 0  # Reset counter if different letter is detected

    # Append letter to word if it's been detected for 10 consecutive frames
    if consecutive_frames == 25:
        word += hand_sign_text  # Add detected letter to the word
        consecutive_frames = 0  # Reset the counter to avoid duplicate entries

    # Update previous detected letter
    prev_sign = hand_sign_text

    print("hand_sign_text: ", hand_sign_text)
    print("word: ", word)
